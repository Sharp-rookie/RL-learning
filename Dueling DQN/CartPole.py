...
class DQN():
        ...
        with tf.variable_scope('target_net'):
            ...
            with tf.variable_scope('Advantage'):
                ...
                self.target_Q_value = self.VT + \
                    (self.AT - tf.reduce_mean(self.AT, axis=1, keep_dims=True))

        t_params = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope='target_net')
        e_params = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope='current_net')
    ...
    def create_training_method(self):
        ...
        self.y_input = tf.placeholder("float", [None])  # Q target
    ...
    def train_Q_network(self):
        ...
        # Step 2: calculate y
        y_batch = []
        Q_value_batch = self.target_Q_value.eval(
    ...
    # output the epsilon-greedy action
